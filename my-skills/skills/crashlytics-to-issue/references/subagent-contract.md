...
- grace_hours: <config.regression.grace_hours>
...
| `last_seen_at` | 회귀 판정용 | `gh.closedAt + grace`와 비교 |
...
| `rendered_title`, `rendered_body` | 완성된 제목·본문 | 통합·회귀가 아니면 **그대로** `gh issue create`로 전달 |
...
- `app_versions`, `os_versions` 범위 (rendered_body 안에만)
...
- **회귀(`REGISTER(regression)`)**: body 상단에 회귀 경고 블록(issue-template.md#Regression-Rendering) prepend. `{previous_issue_number}` / `{previous_issue_url}` / `{closed_at}` / `{last_seen_at}`은 filter-rules.md의 분류 결과 context에서 주입.
